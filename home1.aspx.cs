...
public partial class home1 : System.Web.UI.Page
{
                        ...
                        if (role_type == "Conducting CDF")
                        {
                            ...
                            if (role == "ShadowCDF")
                            {
                                ...
                                else if (cdfacceptance == "True")
                                {
                                    ...
                                    Gridmysession.Columns[21].Visible = true; //accept
                                    Gridmysession.Columns[20].Visible = false; //MSG
                                    Gridmysession.Columns[22].Visible = false;//reject
...

    {
        if (e.CommandName == "Accept")
        {
            ...
            //Access Cell values.
            int studid = int.Parse(row.Cells[6].Text);
        ...
        else
        {
            if (e.CommandName == "Reject")
            {
                ...
                int stuid = int.Parse(row1.Cells[6].Text);
                using (SqlConnection con = new SqlConnection(connStr))
                {
                    SqlCommand cmd = new SqlCommand("sp_MySessionDetaislForCDF", con);
                    cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void Gridmysession_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            else if (e.Row.Cells[1].Text == "ShadowCDF")
            {
                ...
                {
                     if (e.Row.Cells[20].Text == "Awaiting CDF Confirmation")
                    {
                        Gridmysession.Columns[21].Visible = true;
                        e.Row.Cells[20].Text = "CDF Accepted";

                        if(e.Row.Cells[20].Text== "CDF Accepted")
                        {
